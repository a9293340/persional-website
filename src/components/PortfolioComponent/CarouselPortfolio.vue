...
const idx = ref(props.lens.length - 1);
const win_css = ref({});
...
const mobile_move = (type) => {
	...
	dom.style.transform = `translateZ(-1200px) rotateY(-${
		(360 / props.lens.length) * (idx.value + type)
	}deg) `;
...
.carousel-portfolio {
	...
	.card-box {
		...
		.card {
			@apply p-8 bg-slate-200 bg-opacity-80 lg:bg-opacity-40 backdrop-blur-sm duration-700 absolute left-0 right-0 top-0 bottom-0 w-full h-full flex justify-center items-center;
		}
		.card-move {
			@apply bg-opacity-80;
